...
app.get('/survey', function(req,res) {
    ...
    sql2  = "SELECT ID, USER_KEY, NICKNAME_D, '2' AS SUCCESS";
    sql2 += " FROM USER_KEY";
    sql2 += " WHERE USER_KEY = '" + req.query.key + "';";
    ...
    con.query(sql2, [],function(err, result, fields) {
        if (err) {
            ...
            if (result[0] == "" || result[0] == null || result[0] == undefined || result[0] == "null") {
                res.send("<script> alert('키 값을 확인 및 발급해주세요.[메인화면으로 이동합니다.]'); location.href='/'; </script>");
                return;
            } else {
                let sql3 = "";
...
app.get('/key_confirm', function(req,res) {
    var requestIp = require('request-ip');
    ...
    con.query(sql3, nick_value,function(err, result, fields) {
        if (result[0].COUNT == 0) {
            ...
            if (update == "UPDATE") {
                ...
                valueArray[6] = hobby2;
                valueArray[7] = hobby3;
                valueArray[8] = USER_ID;
                valueArray[9] = USER_KEY;
            } else {
                sql  = "INSERT INTO SURVEY";
                sql += "(USER_KEY, IP, ID, NICKNAME_D, NICKNAME_P, LEVEL, POSITION_CD, DROUGHTY_CD, PREFERENCE_ARMY1, PREFERENCE_ARMY2, PREFERENCE_ARMY3)";
                sql += " VALUES";
                sql += "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

                valueArray[0] = USER_KEY;
                valueArray[1] = ip;
                        ...
                        con.query(sql2, valueArray2,function(err, result, fields) {
                            if (err) {
                                ...
                                if (index == keyLen) {
                                    ...
                                    con.query(sql3, valueArray3,function(err, result, fields) {
